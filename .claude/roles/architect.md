...
- 2026-08-20 — Before widening a rule onto a broader concept, MEASURE what the markup declares: #500's "a dropdown should take Escape too" had no oracle at all — naive's dropdown and select popovers carry no ARIA role, so the generalisation could only have been a library class name in the query or a per-component registry, which is exactly what the narrower rule was written to replace.
- 2026-08-20 — Ask the RUNNING page which affordances forward nothing before deciding a defect is general: CDP's `DOMDebugger.getEventListeners` walks an element's ancestors and answers "does anything handle a click here", which reduced #506's "is this the only one" from a template grep to a measurement — 1 of 294 pointer-cursor elements, and the other 293 stayed quiet.
